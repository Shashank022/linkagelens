...
public final class LinkageLensMojo extends AbstractMojo {
    ...
    @Override public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            ...
            Reporter reporter = switch (format.toLowerCase(Locale.ROOT)) {
                case "text" -> new TextReporter(); case "json" -> new JsonReporter(); case "sarif" -> new SarifReporter();
            ...
            if ("text".equalsIgnoreCase(format)) for (String line : rendered.split("\\R")) { if (!line.isBlank()) getLog().info(line); }
            else {
                ...
                Files.writeString(report, rendered); getLog().info("LinkageLens report: " + report);
            }
        ...
        catch (Exception e) { throw new MojoExecutionException("LinkageLens analysis failed: " + e.getMessage(), e); }
    }
    ...
    private List<Path> runtimeRoots() throws DependencyResolutionRequiredException {
        ...
        if (artifact != null && artifact.isFile() && artifact.getName().endsWith(".jar")) roots.add(artifact.toPath().toAbsolutePath().normalize());
        else if (project.getBuild().getOutputDirectory() != null) { Path p = Path.of(project.getBuild().getOutputDirectory()).toAbsolutePath().normalize(); if (Files.exists(p)) roots.add(p); }
        if (includeDependencies) for (String element : project.getRuntimeClasspathElements()) { Path p = Path.of(element).toAbsolutePath().normalize(); if (Files.exists(p)) roots.add(p); }
    ...
    private Severity parseThreshold(String value) throws MojoExecutionException {
        ...
        try { return Severity.parse(value); } catch (IllegalArgumentException e) { throw new MojoExecutionException("Invalid linkagelens.failOn: " + value); }
    }
}
